...
describe('UI', () => {
  it('Calling the login creator with random email and password return the expected result', () => {
    ...
    expect(action).toEqual({type: LOGIN, user: {email: 'email@gmail.com', password: 'password', isLoggedIn: true}});
  });
...
describe('loginRequest', () => {
  ...
  it('creates FETCH_DATA_SUCCESS when fetching data has been done', () => {
    ...
    const expectedActions = [
      { type: LOGIN , user: {email: 'email@gmail.com', password: 'password', isLoggedIn: true}},
      { type: LOGIN_SUCCESS }
    ];
