...
describe('Notifications Component', () => {
  ...
  it('first <NotificationItem> element renders the right html"', () => {
    ...
    const firstChild = ulElement.find('li').first();
    const attr = firstChild.prop('data-notification-type');
    expect(attr).toEqual('default');
    const cls = firstChild.prop('className');
    expect(cls.startsWith('defaultStyle_')).toBe(true);
  });
  ...
  it("Verify that clicking on the menu item calls handleDisplayDrawer and clicking on the button calls handleHideDrawer", () => {
    const handleDisplayDrawer = jest.fn();
    const handleHideDrawer = jest.fn();
    let wrapper = shallow(<Notifications handleDisplayDrawer={handleDisplayDrawer} listNotifications={listNotifications} />);
    wrapper.find('div').first().simulate('click');
    expect(handleDisplayDrawer).toHaveBeenCalled();
    wrapper = shallow(<Notifications displayDrawer={true} handleHideDrawer={handleHideDrawer} listNotifications={listNotifications} />);
